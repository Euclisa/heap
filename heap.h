...
#define __hp_in_heap(heap,i) ((i <= hp_size(heap)) && (i > 0))

#define __hp_left_child(i) (2*i)
#define __hp_right_child(i) (2*i+1)
#define __hp_parent(i) (i/2)
#define __hp_root 1

#define __hp_swap(heap,i,j)                                                                                     \
...
{                                                                                                               \
    ...
    if(!__hp_in_heap(heap,i) && ret == HP_SUCCESS) ret = HP_INDEX_OUT_OF_RANGE;                                 \
    if(ret == HP_SUCCESS)                                                                                       \
        while(1)                                                                                                \
        {                                                                                                       \
            long largest = i;                                                                                   \
            if(__hp_in_heap(heap,__hp_left_child(i)) && hp_compare(heap[__hp_left_child(i)],heap[largest]))     \
                largest = __hp_left_child(i);                                                                   \
            if(__hp_in_heap(heap,__hp_right_child(i)) && hp_compare(heap[__hp_right_child(i)],heap[largest]))   \
                largest = __hp_right_child(i);                                                                  \
            if(largest != i)                                                                                    \
            {                                                                                                   \
                __hp_swap(heap,largest,i)                                                                       \
                i = largest;                                                                                    \
            }                                                                                                   \
            else                                                                                                \
                break;                                                                                          \
...
{                                                                                                               \
    long i = ind;                                                                                               \
    ret = HP_SUCCESS;                                                                                           \
    if(!__hp_in_heap(heap,i)) ret = HP_INDEX_OUT_OF_RANGE;                                                      \
    if(hp_compare(heap[i],key) && i != (hp_size(heap)) && ret == HP_SUCCESS) ret = HP_INVALID_KEY;              \
    if(ret == HP_SUCCESS)                                                                                       \
    {                                                                                                           \
        heap[i] = key;                                                                                          \
        while(i > __hp_root && hp_compare(heap[i],heap[__hp_parent(i)]))                                        \
        {                                                                                                       \
            __hp_swap(heap,i,__hp_parent(i))                                                                    \
            i = __hp_parent(i);                                                                                 \
        }                                                                                                       \
    }                                                                                                           \
}
...
{                                                                                                               \
    ...
    {                                                                                                           \
        out = heap[__hp_root];                                                                                  \
        heap[__hp_root] = heap[hp_size(heap)];                                                                  \
        hp_size(heap)--;                                                                                        \
        if(!hp_empty(heap))                                                                                     \
            hp_heapify(heap,__hp_root,ret);                                                                     \
    }                                                                                                           \
}
